...
from op2_gen_common_post import *  #Modify1
...
def op_decl_const_parse(text):
  ...
  for m in re.finditer('op_decl_const\((.*)\)', text):
    ...
    # check for syntax errors
    if len(args) != 3 and len(args) != 4:#Modify2
      print('Error in op_decl_const : must have three arguments')
      return
    #Modify2###
    gbl_flag=0
    if len(args) == 4:
      gbl_flag = args[3].strip()
    #Modify2###
    consts.append({
      'loc': m.start(),
      'dim': args[0].strip(),
      'dim2':args[0].strip(), #Modify2
      'gbl_flag':gbl_flag,#Modify2
      'type': args[1].strip(),
      'name': args[2].strip(),
      'name2': args[2].strip()
      })
...
def main(srcFilesAndDirs=sys.argv[1:]):
  ...
  list_h_cpp1=[]
  for i in temp_list:
    if((len(i)>2 and i[-2:]=='.h') or (len(i)>4 and i[-4:]=='.cpp')):
      list_h_cpp1.append(i)
  for src_file in list_h_cpp1:
  #Modify3
  ##########
  # for src_file in src_files:
    print(("Parsing file '" + src_file + "' for macro definitions."))
    with open(src_file, 'r',encoding='raw_unicode_escape') as f:
      text = f.read()
    text = op2_gen_common.comment_remover(text)
  ...
  for src_file in src_files:
    ...
    with open(src_file, 'r') as f:
      text = f.read()
    text = op2_gen_common.comment_remover(text) #Modify4 
    any_soa = 0
    ...
    for i in range(0, len(const_args)):
      ...
      if repeat == 0:
        ...
        temp = {'dim': const_args[i]['dim'],
            'dim2':const_args[i]['dim2'], #Modify2
            'gbl_flag':const_args[i]['gbl_flag'], #Modify2
    ...
    for loc in range(0, len(locs)):
      ...
      if locs[loc] in loc_consts:
        ...
        #Modify2 
        # print("Modify2:\n")
        for consts_dim2 in range(len(const_args[curr_const]['dim2'].split('*'))-1):
          fid.write('*')
        fid.write(const_args[curr_const]['name2'].strip()+ ');')
        #Modify2 
        loc_old = endofcall + 1
        continue
  ...
  if(os.environ.get('OP_USER_DATATYPES')!=None):
    gen_user_datatype(kernel_names)
    gen_user_datatype_mpi(macro_defs)
    ##########
