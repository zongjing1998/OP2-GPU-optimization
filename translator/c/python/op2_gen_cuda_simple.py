...
import op2_gen_common_post #Modify1
...
##########################################################################
    ...
    print(kernels[nk]['name']) 
    print('maps:',maps)
    print('mapnames:',mapnames)
    print('inds:',inds)
    print('inddims:',inddims)
    print('indaccs:',indaccs)
    print('invinds:',invinds)
    print('invmapinds:',invmapinds)
    print('mapinds:',mapinds)
    print('nmaps:',nmaps)
    print('ninds:',ninds)
    print('nargs:',nargs)
    print('')
    ...
    for g_m in range(0,ninds):
      if (indaccs[g_m]==OP_READ):
        ...
        #Modify5
        temp_flag=0
        for temp_i in range(0,nargs):
          if(temp_var[invinds[g_m]]==temp_var[temp_i] and accs[temp_i]!=OP_READ):
            temp_flag=1
            break
        if temp_flag==1:
...
##########################################################################
  ...
  file_text = ''
  code('#include "user_cuda.h"') #Modify6
  comm('global constants')
  
  code('#ifndef MAX_CONST_SIZE')
  code('#define MAX_CONST_SIZE 2500')
  code('#endif')
  code('')
  ...
  for nc in range (0,len(consts)):
    #####################################
    #Modify7
    type_device='__constant__ ' #Modify7
    if consts[nc]['gbl_flag']=='1':
      type_device='__device__ '
    if(consts[nc]['dim2'].find('*')!=-1):
      dim2_string=''
      for consts_dim2 in consts[nc]['dim2'].split('*'):
        dim2_string+='['+consts_dim2.strip()+']'
      code(type_device+consts[nc]['type'][1:-1]+' '+ consts[nc]['name']+'_cuda'+dim2_string+';')
      continue
    #Modify7
    ######################################
    
    if consts[nc]['dim']==1:
      code(type_device+consts[nc]['type'][1:-1]+' '+consts[nc]['name']+'_cuda;')
    else:
      ...
      code(type_device + consts[nc]['type'][1:-1]+' '+consts[nc]['name']+'_cuda['+num+'];')
  code('')
  ...
  for nc in range(0,len(consts)):
    ...
    #Modify8
    # if(consts[nc]['dim2'].find('*')!=-1):
    #   size_string=consts[nc]['dim2']+' * sizeof( ' + consts[nc]['type'][1:-1]+')'
    #   code('cutilSafeCall(cudaMemcpyToSymbol('+consts[nc]['name']+'_cuda, dat, '+size_string+'));')
    # #Modify8
    # ######################################
    # else:
  ...
  op2_gen_common_post.op2_gen_cuda_post(kernel_names) #Modify9
