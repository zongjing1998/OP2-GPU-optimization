...
##########################################################################
  ...
  code('#endif')
  code('')
  code('#include "user_openmp.h"') #Modify2
  comm(' global constants       ')

  for nc in range (0,len(consts)):
    if not consts[nc]['user_declared']:
      temp_const=''
      if op2_gen_common_post.op2_gen_openmp_extern(kernel_names,consts[nc]['name']):
        temp_const='const '
      if(consts[nc]['dim2'].find('*')!=-1):  #Modify3
        dim2_string=''
        for consts_dim2 in consts[nc]['dim2'].split('*'):
          dim2_string+='['+consts_dim2.strip()+']'
        code('extern '+temp_const+consts[nc]['type'][1:-1]+' '+consts[nc]['name']+dim2_string+';')
        continue
      if consts[nc]['dim']==1:
        code('extern '+temp_const+consts[nc]['type'][1:-1]+' '+consts[nc]['name']+';')
      else:
        ...
        code('extern '+temp_const+consts[nc]['type'][1:-1]+' '+consts[nc]['name']+'['+num+'];')
  code('')
  ...
  op2_gen_common_post.op2_gen_openmp_post(kernel_names) #Modify4
